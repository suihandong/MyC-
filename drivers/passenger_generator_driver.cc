...
int main() {
  ...
  PassengerGenerator * generator = new RandomPassengerGenerator(probs, my_stops);
  ...
  for (std::list<Stop *>::iterator stop_iter = my_stops.begin(); stop_iter != my_stops.end(); stop_iter++) {
    (*stop_iter)->Update();
    myfile  << std::endl << "\t*** Stop Report ***" << std::endl << std::endl;
    (*stop_iter)->Report(myfile);
  }
  ...
  myfile << "*\n*\n*\n*\n*\t PASSENGERS CREATED *\n*\n*\n*\n*\n" << std::endl;
  myfile << total_generated << std::endl;
  //myfile.close();
  return 0;
}
